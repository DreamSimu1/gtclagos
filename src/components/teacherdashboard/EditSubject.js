...
const EditSubject = ({
  ...
  useEffect(() => {
    if (selectedSubject) {
      ...
      setTeacherUsername(selectedSubject.teacher?.username || "");
      setClassname(selectedSubject.classname || "tech_1");
    }
  }, [selectedSubject]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${apiUrl}/api/subject/update-subject/${selectedSubject._id}`,
        {
          name: subjectName,
          teacherUsername,
          classname,
      ...
      alert("Failed to update subject. See console for details.");
    }
  };
