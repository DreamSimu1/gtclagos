...
const Chat = ({ isNewChat = false }) => {
  ...
  const [chatId, setChatId] = useState(paramChatId || uuidv4());
  ...
  // const chatTitle = "Investment Chat";
  ...
  useEffect(() => {
    if (isNewChat) {
      ...
      setChatId(newId);
      setMessages([]);
      setChatTitle("Chat " + newId.slice(0, 6));
      navigate(`/chat/${newId}`);
    } else if (paramChatId) {
      fetchChat(paramChatId);
    }
  }, [paramChatId, isNewChat]);

  const fetchChat = async (id) => {
    try {
      const res = await axios.get(`${apiUrl}/api/ai/fin-inquiry/chat/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
        },
      });

      const chatMessages = res.data.data.flatMap((msg) => [
  ...
  //   formData.append("chatTitle", "Chat " + chatId.slice(0, 6)); // Example
  ...
  //   try {
  //     const res = await axios.post(`${apiUrl}/api/ai/fin-inquiry`, formData, {
  //       headers: {
  //         Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
  //       },
  //     });

  //     const aiText = res.data.data.aiResponse;
